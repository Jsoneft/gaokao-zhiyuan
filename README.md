...
- **数据库表结构**: 添加完整的ClickHouse表结构说明
- **工具程序说明**: 详细分析tools目录下各工具的功能和保留建议
- **配置文档**: 完善环境变量配置说明

#### 🛠️ 工具优化
- **工具分类**: 将tools目录下的工具按功能分类
- **保留建议**: 为每个工具提供明确的保留或删除建议
- **功能说明**: 详细说明每个工具的用途和使用场景

#### 🏗️ 架构改进
- **表结构优化**: 使用新的admission_hubei_wide_2024表提升查询性能
- **兼容性保持**: 保留旧表结构确保向后兼容
- **索引优化**: 优化数据库索引提升查询效率
